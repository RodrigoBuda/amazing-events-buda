...
function statsInitializer() {
  ...
  eventStatics.innerHTML = eventStaticsShow(
    sortEventByPorcentageOfAttendance,
    sortEventBycapacity
  );
  ...
  function eventStaticsShow(eventsAttendance, eventsCapacity) {
    ...
    for (let i = 0; i < 3; i++) {
      rows += `<tr>
                    <td>${eventsAttendance[i].name} : ${(
        (eventsAttendance[i].assistance / eventsAttendance[i].capacity) *
        100
      ).toFixed(2)} %</td >
                    <td>${
                      eventsAttendance[eventsAttendance.length - i - 1].name
                    } : ${(
        (eventsAttendance[eventsAttendance.length - i - 1].assistance /
          eventsAttendance[eventsAttendance.length - i - 1].capacity) *
        100
      ).toFixed(2)} %</td >
                    <td>${eventsCapacity[i].name} : ${
        eventsCapacity[i].capacity
      }</td >               
                    </tr>`;
    }
    return rows;
